...
drop1(update(glm_drop, as.formula(paste(".~.- cover - duration"))), test = "LRT")
drop1(update(glm_drop, as.formula(paste(".~.- cover - duration - period"))), test = "LRT")
```
...
# Recipe
rec <- 
  recipe(mortality ~ sex + smoker + good_health + actuarial_age + capital + IMC, data = insurance_train) %>%
...
For instance, we can observe that if you were a woman, the odds of mortality are 0.64 times lower than if you were a man. However, this model is saying that if you had good health, the odds of mortality are 2.26 times bigger than if you had a bad health, which does not make much sense.
...
# Formula
rec <- recipe(mortality ~ sex + smoker + good_health + actuarial_age + capital + IMC, data = insurance_train)
