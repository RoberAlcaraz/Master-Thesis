...
set.seed(23)
lapse_folds <- vfold_cv(lapse_train, v = 10, strata = lapse)
```
...
my_metrics = metric_set(f_meas, kap, recall)
```
...
kknn_rec <- 
  ...
  step_interact(terms = ~ duration:sex_woman + actuarial_age:sex_woman + 
...
# SVM
set.seed(123)
max_ent_svm <- grid_max_entropy(parameters(svm_wf), size = 10)
...
# Decision tree
set.seed(123)
max_ent_dt <- grid_max_entropy(parameters(dt_wf), size = 10)
